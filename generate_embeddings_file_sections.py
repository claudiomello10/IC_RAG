...
def generate_chunks_Aurelien(
    ...
    for i in range(4, 13):
        page_text = reader_aurelien.pages[i].extract_text()
        ...
        for line in page_text:
            if re.match(r"^\d+\.", line):
                ...
                if result:
                    ...
                    page = topic_text[-1]
                    page = int(page)
                    summary_dict[chapter]["topics"].append(
    ...
    for chapter in tqdm(summary_dict):
        ...
        for topic in topics:
            ...
            if topic == topics[-1]:
                if chapter == 14:
                    next_topic_page = len(reader_aurelien.pages)
                else:
                    next_topic_title = summary_dict[chapter + 1]["topics"][0]["Topic"]
                    next_topic_page = (
                        summary_dict[chapter + 1]["Page"] + page_correction
                    )
            else:
                next_topic_page = (
                    topics[topics.index(topic) + 1]["Page"] + page_correction
                )
...
# Dictionary with the book function to generate the chunks
BOOK_FUNCTION_DICT = {
    "Aurelien": generate_chunks_Aurelien,
}


# Generate the chunks for each book
df = pd.DataFrame(columns=["Chapter", "Title", "Text", "Topic"])
for book in BOOK_FUNCTION_DICT:
    print(f"Generating chunks for the book {book}")
    df = df.append(BOOK_FUNCTION_DICT[book](), ignore_index=True)
